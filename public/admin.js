...
const admin = {
  ...
  async refreshAll() {
    await Promise.all([this.loadDashboard(), this.loadProducts(), this.loadOrders()]);
  },
  async loadDashboard() {
  ...
  async syncRichMenu() {
    if (!confirm('確認要同步三頁 LINE Rich Menu？這會刪除本專案舊的 WW Beyblade V7 Rich Menu 後重新建立。')) return;
    const el = $('#richmenuStatus');
  ...
  async deleteRichMenu() {
    if (!confirm('確認刪除本專案建立的 WW Beyblade V7 Rich Menu？')) return;
    const el = $('#richmenuStatus');
