      ...
      box.innerHTML = `
        <div class="order-card">
          <div style="display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap">
            ...
            <span class="tag">更新：${new Date(latest.uploadedAt).toLocaleString()}</span>
          </div>
          ${latest.note ? `<p>${latest.note}</p>` : ''}
          <img src="${latest.imageUrl}" alt="最新賠率" style="width:100%;border-radius:28px;margin-top:10px;box-shadow:var(--shadow);border:1px solid rgba(15,23,42,.08)" />
        </div>`;
    } catch (err) {
      $('#oddsBox').textContent = err.message;
    }
  },
