...
- ZIP 已改為根目錄封包，Zeabur `Add files via upload` 不會再因外層資料夾造成舊檔覆蓋不完整。
- 後台商品圖片欄位改成「可直接上傳本地端」，並加上 admin.js cache-busting。
- Rich Menu 後台仍保留一鍵同步、查詢狀態、刪除本專案 Rich Menu。部署後請進入後台執行「一鍵同步 Rich Menu」。


## V7.0.5 後台記帳 / Google Sheet 同步

新增後台「記帳紀錄 / Google Sheet」功能：
...
訂單建立會自動新增消費紀錄；確認銀行匯款會自動新增入帳紀錄。

需要同步 Google Sheet 時，請設定：
...
詳細設定請看 `docs/GOOGLE_SHEET_ACCOUNTING.md`。


## V7 Rich Menu 修正
- 修正 LINE Rich Menu 圖檔上傳 endpoint，改用 `https://api-data.line.me/v2/bot/richmenu/{richMenuId}/content`。
- 解決後台同步時出現 `POST /v2/bot/richmenu/.../content failed 404` 的問題。
